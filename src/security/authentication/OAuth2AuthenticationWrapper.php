...
class OAuth2AuthenticationWrapper extends AuthenticationWrapper
{
    ...
    private function login($driverName, $element, CsrfTokenDetector $csrf)
    {
        ...
        if (!empty($_GET["code"])) {
            ...
            if (!$targetSuccessPage) {
                $targetSuccessPage = self::DEFAULT_TARGET_PAGE;
            }
            $targetFailurePage = (string) $this->xml["login"];
            if (!$targetFailurePage) {
                $targetFailurePage = self::DEFAULT_LOGIN_PAGE;
            }
            ...
            if ($scopes) {
                $targetScopes = array_merge($targetScopes, explode(",", $scopes));
            }
