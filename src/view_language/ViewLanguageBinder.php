...
class ViewLanguageBinder
{
    ...
    private function setCompilationFile(\SimpleXMLElement $xml, $viewFile)
    {
        ...
        if (!$compilationsFolder) {
            throw new \Lucinda\MVC\STDOUT\XMLException("Tag 'compilations' child of 'paths' child of 'application' tags is empty or missing");
        }
        $tagsFolder = (string) $xml->paths->tags;
        $viewsFolder = (string) $xml->paths->views;
        $extension = (string) $xml["templates_extension"];
        if (!$extension) {
            throw new \Lucinda\MVC\STDOUT\XMLException("Attribute 'templates_extension' is missing for 'application' tag");
        }
